...
NanostackRfPhyAtmel::NanostackRfPhyAtmel(PinName spi_mosi, PinName spi_miso,
    ...
    : _mac(i2c_sda, i2c_scl), _mac_addr(), _rf(NULL), _spi_mosi(spi_mosi),
      _spi_miso(spi_miso), _spi_sclk(spi_sclk), _spi_cs(spi_cs), _spi_rst(spi_rst),
      _spi_slp(spi_slp), _spi_irq(spi_irq)
{
...
void NanostackRfPhyAtmel::rf_read_mac_address(uint8_t *mac)
{
    rf_if_lock();
